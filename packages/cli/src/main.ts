...
import type { Diagnostics } from "@bomdd/core";
...
function computeExit(diag: Diagnostics, schema: Parameters<typeof appliedRules>[2], failOn: "error" | "warn"): number {
  const applied = appliedRules(diag.run.gate, diag.run.eco, schema);
  let hasError = false;
  let hasWarn = false;
  for (const f of diag.findings) {
    if (!applied.has(f.rule)) continue;
    if (f.suppressed) continue;
    if (f.severity === "error") hasError = true;
    else if (f.severity === "warn") hasWarn = true;
  }
...
export function main(argv: string[]): number {
  ...
  try {
    ...
    try {
      ...
      if (args.view) {
        const html = generateView(
          canonicalJson(result.diagnostics),
          canonicalJson(result.graph),
