...
export function isValidGate(g: string): boolean {
  return VALID_GATES.includes(g);
}

/** X-* implementation diagnostics are all gate=always (§2.6). */
export function gateOfRule(ruleId: string, schema: RefSchema): string {
  if (ruleId.startsWith("X-")) return "always";
  const r = schema.lintRules.find((x) => x.id === ruleId);
  return r?.gate ?? "always";
}

/** Compute the set of rule IDs applied under a gate (+ eco). */
export function appliedRules(gate: string, eco: boolean, schema: RefSchema): Set<string> {
