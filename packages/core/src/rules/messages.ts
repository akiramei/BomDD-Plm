...
const TABLE: Record<string, MessageTemplate> = {
  "R-001": {
  ...
  "R-020": {
    message:
      "UI surface 品目 {targetId} が workspace 内のどの ui-trace-map からも参照されていません(CAD なし製造の予防)",
    fixTarget: "実装ではなく bomdd/ui/ の UI-IR と trace map を先に作成する",
  },
  "R-021": {
    message: "Design System 部品 {targetId} が 30-ebom.yaml に同期されていません",
    fixTarget:
      "30-ebom.yaml に E-DESIGN 品目を追加するか、coverage_matrix で理由付き out-of-scope を宣言する",
  },
  "R-030": {
...
function substitute(tpl: string, vars: MessageVars): string {
  return tpl.replace(/\{(targetId|file|family|ref|rule|supIndex)\}/g, (_m, key: string) => {
...
export function getMessage(rule: string, vars: MessageVars = {}): MessageTemplate {
...
export function hasMessage(rule: string): boolean {
 ...
 */
export function getRawMessageTemplate(rule: string): string {
  const tpl = TABLE[rule];
  if (!tpl) {
    throw new Error(`rule-messages.yaml に規則 ${rule} の文言がありません(仕様の穴 blocker)`);
  }
