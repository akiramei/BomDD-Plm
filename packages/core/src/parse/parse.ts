 ...
 */
export function firstInvalidUtf8Offset(buf: Buffer): number {
  const decoded = buf.toString("utf8");
  const reencoded = Buffer.from(decoded, "utf8");
  if (reencoded.equals(buf)) return -1;
  // find first differing byte
  const n = Math.min(buf.length, reencoded.length);
  for (let i = 0; i < n; i++) {
    if (buf[i] !== reencoded[i]) return i;
  }
  return n;
}

/** Compute 1-based line/column (code point units) for a byte offset in a buffer. */
function lineColForByteOffset(buf: Buffer, byteOffset: number): { line: number; column: number } {
  let line = 1;
  let column = 1;
...
function hintForLine(sourceLine: string | undefined): string | undefined {
  if (sourceLine === undefined) return undefined;
  // Trap 1: list item beginning with `*`
  if (/^\s*-\s+\*/.test(sourceLine)) return CANON_TRAP_HINTS.star;
  // Trap 3: item that starts with a quote but has trailing text after the closing quote
  if (/^\s*-\s+(["']).*?\1\s*\S/.test(sourceLine)) return CANON_TRAP_HINTS.quote;
...
function mkFinding(rule: string, file: string, ref: string, extra: Partial<Finding> = {}): Finding {
 ...
 */
export function parseArtifact(art: Artifact, raw: Buffer): ParseResult {
  ...
  if (badOffset >= 0) {
    ...
    findings.push(
      mkFinding("X-PARSE-001", art.canonicalPath, `不正な UTF-8 バイト列(byte offset ${badOffset})`, {
        line,
        column,
      })
    );
  ...
  const dupKeyErrors = doc.errors.filter((e) => (e as { code?: string }).code === "DUPLICATE_KEY");
  const syntaxErrors = doc.errors.filter((e) => (e as { code?: string }).code !== "DUPLICATE_KEY");
  ...
  if (syntaxErrors.length > 0) {
    for (const err of syntaxErrors) {
      ...
      findings.push(
        mkFinding("X-PARSE-001", art.canonicalPath, refMsg, { line: lc.line, column: lc.col })
      );
    }
    return { findings };
  }
  ...
  for (const err of dupKeyErrors) {
    ...
    const lc = lineCounter.linePos(pos);
    findings.push(
      mkFinding("X-TYPE-001", art.canonicalPath, `重複キー: ${err.message}(後勝ち採用)`, {
        line: lc.line,
        column: lc.col,
      })
    );
  ...
  try {
    ...
    findings.push(
      mkFinding("X-PARSE-001", art.canonicalPath, `YAML 構文エラー: ${(e as Error).message}`)
    );
    return { findings };
  }
...
function buildLineLookup(doc: ReturnType<typeof parseDocument>, lineCounter: LineCounter): LineLookup {
  return (path: (string | number)[]): number | undefined => {
