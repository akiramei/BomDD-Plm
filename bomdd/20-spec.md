...
> rev: **rev4**(2026-07-04 — ECO-005: §2.17 diff_audit に head: アンカー(固定窓= stale 恒久修正)+
> X-GIT-001 文言の rev 一般化。ref-v0.8 に追随)
> rev3(2026-07-03 — ECO-002: §2.17 新設 R-052 diff-audit(git 連携・diff_audit opt-in)+
...
### 2.17 R-052 eco-diff-within-impact — diff 監査の機械化 (REQ-010 rev3・rev4/ECO-005)
...
- 振る舞い:
  - **検査対象(opt-in)**: `--eco` 実行時、60-change-register の changes[] のうち **`diff_audit` フィールドを
    持つエントリのみ**。形: `diff_audit: { baseline: <git rev(タグ名/SHA)>, head: <git rev(任意・受入タグ推奨)>,
    allowed_paths: [<リポ内相対プレフィックス>...] }`(head は rev4/ECO-005・ref-v0.8)。
  ...
  - **実 diff の取得**: 当該 register が属するリポで `git diff --name-only <baseline> <head|HEAD>` を実行する
    (読み取り専用サブコマンドのみ。working tree でなくコミット間 diff — 採点の再現性のため)。
  - **diff 窓の終端(rev4/ECO-005 = stale 恒久修正)**: `head` **有り**= 固定窓 baseline..head。HEAD 非依存で
    恒久決定的 — verified 後の無関係コミットが窓に入らず、宣言影響集合が遡及的に破れない(status に
    関わらず常に評価= 恒久回帰検査)。`head` **無し**= 従来どおり baseline..HEAD(open ECO の生きた監査窓)。
    **運用**: verified 時は diff_audit を除去せず **head: に受入タグを追記して窓を閉じる**。
    (status によるゲートは非採用 — status は ref-v0 未規定語彙であり、意味論を未規定次元に結合させない。
    裁定= ECO-005 order §2)
  ...
  - **fail-open**: git 実行不能(コマンド不在・非 git リポ)または baseline/head 解決不能の場合は **X-GIT-001(info)** を
    ECO ごとに1件発し(メッセージの {ref} は head 有りなら窓表記 `baseline..head`)、当該 ECO の R-052 判定を
    skip する(検査不能でコミット/CI を wedge しない。X-XREPO と同型)。
