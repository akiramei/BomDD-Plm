...
> rev: **rev2**(2026-07-03 — ECO-001: §2.4 パス受理形3種の明文化(PD-7/PD-8)+§2.5 per-file 一意性スコープ。
> ref-v0.4 に追随)
...
- 振る舞い:
  ...
  - **参照解決**(kind 語彙は ref-v0.1 凡例と一致):
    ...
    - **パス値の受理形(rev2・ref-v0.4。kind: path / id-or-path の② / path-at-rev のパス部に共通)**:
      ...
      3. repo: 形 `<repo名>:<リポ内相対>` — 正準形と**等価**(`:` を `/` に読み替える)。repo 名が workspace に不在なら不解決
         (形式2 へはフォールバックしない — repo 名の明示は書き手の意図)。
