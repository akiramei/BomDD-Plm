...
- **ViewPrism2 ECO 候補**: 6件(上表)。着手は ViewPrism2 側の通常 ECO プロセスで。
  ...
  → **実装完了(2026-07-03・同日)**: 028/029/030/031/032(a)/033/034 全て applied。
  **workspace lint error 250(受入時)→124(S-19 裁定後)→0**・warn 156→85(残85は全て 032(b) 便乗対象の債務)。
  実装検証で裁定の**誤認2件を訂正**: (1) ECO-030 の前提「FMEA 構造表が無い」— 実は 32-mbom に36定義が実在し、
  真因は ref-v0.3 の定義サイト規定(33のみ=自リポの書き癖)→ **ref-v0.5(実物が正・6回目)** で解消・ViewPrism2 は
  逆参照1行のみ。(2) trace-map 複合記法の正規化は単純配列化では**検査が黙って消える**(セレクタが配列を
  スキップ)→ **ref-v0.6** で uiIr[]/uiBom[] 配列エッジを併記。真正欠陥の追加捕獲: ebomItemRef への K 参照混在・
  作業タブモックの取込先パス誤り(ingestStatus=pending のまま実態と乖離)。
